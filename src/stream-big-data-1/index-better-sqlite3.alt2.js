...
db.exec('CREATE TABLE pairs (key VARCHAR(255), pair VARCHAR(255))');
// db.exec('create index idx_pair_key on pairs (key)');
// db.exec('create unique index idx_pair_uniq on pairs (key, value)');

const stmtPairInsert = db.prepare('INSERT INTO pairs (key, pair) values (?, ?)');
...
async function handleFile(filepath) {
  ...
  for await (const line of rl) {
    if (line !== undefined && line !== null) {
      ...
      const [key] = line.replace(/^\s+|\s+$/g, '').split(/: /);
      stmtPairInsert.run(key, line);
    }
  ...
  // result.uniq_pairs = db.prepare('select count(*) as n from (select 1 from pairs group by pair)').get()?.n || 0;
  // console.timeEnd('3');
  await Promise.all([
    Promise.resolve().then(() => db.prepare('select count(*) as n from (select 1 from pairs group by key)').get()?.n || 0),
    Promise.resolve().then(() => db.prepare('select count(*) as n from (select 1 from pairs group by pair)').get()?.n || 0)
  ]);
